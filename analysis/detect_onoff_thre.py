...
class MUA_findchangepts:
    ...
    def __init__(self):
        ...
        self.MaxNumChanges = None # maximum numbers of on-off state change points
        self.MinDistance = 5 # minimum distance between two change points; in multiple of 'mua_sampling_interval';
...
def on_off_properties(data, cpts, method = "comparing", threshold = None):
    ...
    if method == "threshold":
        ...
        return onoff_bool, on_t, off_t, on_amp, off_amp, onset_t, offset_t
#%%
def get_best_onoffThreshold(data, cpts, threshold_list = None):
    ...
    for thre_i in range(len(threshold_list)):
        ...
        cpts_i = np.hstack(([0],cpts_i,mean_data.shape)).astype(int)
        #print(cpts_i)
